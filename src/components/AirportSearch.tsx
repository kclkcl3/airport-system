...
const AirportSearch: React.FC<AirportSearchProps> = ({ flightList }) => {
  ...
  const handleSearch = () => {
    if (!destination.trim()) {
      alert('Введите название аэропорта назначения');
      return;
    }

    const foundFlights = flightList.findFlightsByDestination(destination.trim());
      ...
      {searched && (
        <div className="search-results">
          ...
          {flights.length > 0 ? (
            <FlightList 
              flights={flights} 
              title={`Рейсы в ${destination}`}
            />
          ) : (
            <div className="no-results">Рейсы в указанный аэропорт не найдены</div>
          )}
        </div>
      )}
    </div>
