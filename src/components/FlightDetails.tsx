...
interface FlightDetailsProps {
	// пропсы компонента FlightDetails
...
const FlightDetails: React.FC<FlightDetailsProps> = ({ flightList }) => {
	...
	const handleSearch = () => {
		...
		if (!planeNumber.trim()) {
			// если поле ввода пустое (не введен номер самолета) или содержит только пробелы, устанавливаем сообщение об ошибке. .trim() удаляет пробелы в начале и конце строки
		...
		if (foundFlight) {
			// если рейс найден (foundFlight не null), обновляем состояние flight и сохраняем найденный рейс в state. state - состояние компонента
	...
	return (
		// основной контейнер компонента FlightDetails, flight-details-component - CSS класс для стилизации компонента
		<div className='flight-details-component'>
			<h2>Поиск рейса по номеру самолета</h2> {/*// заголовок компонента*/}
			<div className='search-form'>
				{/* // контейнер для формы поиска рейса по номеру самолета, search-form -
				CSS класс для стилизации формы */}
				<input
				...
				<button onClick={handleSearch} className='search-button'>
					{/* // кнопка для запуска поиска рейса по номеру самолета, при клике
					вызывается функция handleSearch из компонента FlightDetails 
          */}
					Найти
				</button>
			</div>
			...
			{/* 
			// error && - если есть сообщение об ошибке (error не пустая строка),
			отображаем его в div с классом error-message для стилизации
      Логика &&:
